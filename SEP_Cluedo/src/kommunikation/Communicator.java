...
public class Communicator {
	...
	public Communicator(CluedoGameClient network) {
		...
		if (network.hasPlayerConnectedByNick(myNick)) ki = new KI(network,pcManager,this);
	...
	/**
	 * Highlighted die Karten, mit denen ein Spieler widerlegen darf.
	 * Wird zusammen mit setCardFunction verwendet
	 * 
	 * @param card die Karte, mit der man widerlegen kann
	 */
	public void highlightCard(String card) {
		...
		for (int i = 0; i < myHand.size(); i++) {			
			if (card.equals(myHand.get(i))) {
				gameView.getHand().getHand().get(i).setEffect(new Glow(1.0));
				gameView.getHand().getHand().get(i).translateYProperty().setValue(
						gameView.getHand().getHand().get(i).getY() - 50);
			}
		}
	}
	...
	/**
	 * Ermöglicht das disproven und entfernt die Effekte von den rest der Karten in der Hand
	 * 
	 * @param card Eine Karte, mit der man widerlegen kann
	 */
			...
			if (card.equals(myHand.get(i))) {
				...
				gameView.getHand().getHand().get(i).setOnMouseReleased(e -> {
					...
					for(int j = 0; j < myHand.size(); j++){
						gameView.getHand().getHand().get(j).translateYProperty().
						setValue(gameView.getHand().getHand().get(j).getY());
					}
				});
			}
		}
...
public void showPoolCards(ArrayList<String> karten) {
	...
	public void setHandler() {
		gameView.getStage().setOnCloseRequest(new EventHandler<WindowEvent>() {
			...
			public void handle(WindowEvent e) {
				try {
					if (network.hasWatcherConnectedByNick(myNick)) gameView.closeChatStage();
					network.kill();
				} catch (Exception e1) {
		...
		public void setServerStateMsg(String str){
			...
			/**
			 * arrangiert welche der Buttons im Zugfenster verwendet werden dürfen 
			 * 
			 * @param states nimmt sich die Liste mit den erlaubten Zustände
			 */
			...
			/**
			 * Überprüft, ob der Spieler gewonnen oder verloren hat
			 * 
			 * @param nick
			 */
			...
			/**
			 * YOU WON WHAT WILL YOU DO WITH THE MONEY??
			 */
	...
	public void moveForSuspiciton(int gameID2, CluedoStatement suspicion) {
		...
		beweger.getCarriedAlongWeapon(network, suspicion.getRoom(), network.getWeaponByName(suspicion.getWeapon().getName()));
		
		if (pcManager.getPlayerByPerson(suspicion.getPerson()) != null){
			CluedoPosition position = pcManager.getPlayerByPerson(suspicion.getPerson()).getPosition();
			
			if (kacheln.getKachelAt(position.getY(), position.getX()).isIstRaum()){
				if (kacheln.getKachelAt(position.getY(), position.getX()).getRaum() != suspicion.getRoom()){
