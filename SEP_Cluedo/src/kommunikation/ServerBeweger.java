...
public class ServerBeweger {
	...
	public CluedoPosition useSecretPassage(CluedoPlayer player){
		...
		if (kacheln.getKacheln()[position.getY()][position.getX()].getRaum()==Rooms.study){
			position.setX(raumBeweger.positionInRaum(Rooms.kitchen).getPosition().getX());
			position.setY(raumBeweger.positionInRaum(Rooms.kitchen).getPosition().getY());
		}
		else if (kacheln.getKacheln()[position.getY()][position.getX()].getRaum()==Rooms.kitchen){
			position.setX(raumBeweger.positionInRaum(Rooms.study).getPosition().getX());
			position.setY(raumBeweger.positionInRaum(Rooms.study).getPosition().getY());
		}
		else if (kacheln.getKacheln()[position.getY()][position.getX()].getRaum()==Rooms.lounge){
			position.setX(raumBeweger.positionInRaum(Rooms.conservatory).getPosition().getX());
			position.setY(raumBeweger.positionInRaum(Rooms.conservatory).getPosition().getY());
		}
		else if (kacheln.getKacheln()[position.getY()][position.getX()].getRaum()==Rooms.conservatory){
			position.setX(raumBeweger.positionInRaum(Rooms.lounge).getPosition().getX());
			position.setY(raumBeweger.positionInRaum(Rooms.lounge).getPosition().getY());
		}
		else {
			return null;
		}
	...
	public void getCarried(Rooms zielRaum, CluedoPlayer player){
		Kachel neuePositionKachel = raumBeweger.positionInRaumMitPlayer(player, zielRaum);
		player.setNewPosition(neuePositionKachel.getPosition());
	}
	
	public void reactToSuspicion(CluedoStatement suspicion){
		if (pcManager.getPlayerByPerson(suspicion.getPerson()) != null){
			...
			if (kacheln.getKachelAt(position.getY(), position.getX()).isIstRaum()){
				if (kacheln.getKachelAt(position.getY(), position.getX()).getRaum() != suspicion.getRoom()){
					...
					Rooms raum = suspicion.getRoom();
					getCarried(raum, player);
				}
			}
			else {
				CluedoPlayer player = pcManager.getPlayerByPerson(suspicion.getPerson());
