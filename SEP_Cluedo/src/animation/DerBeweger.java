...
public class DerBeweger {
	...
	public void bewegen(Kachel spielerPositionKachel, Orientation [] anweisungenEingabe, int schritteEingabe, int nullschritteEingabe){
		...
		if(spielerPositionKachel.isIstTuer()){
			...
			Kachel raumStartKachel = raumBeweger.positionInRaum(room);
...
//		Label spielerPositionLabel = boardView.getLabelHier(spielerPositionKachel);
		...
		System.out.println("player position y : " + player.getPosition().getY() + "  ||  x  : " +player.getPosition().getX());
		
		
		Kachel spielerAnfangsPositionKachel = new Kachel();
		...
		if (spielerPositionKachel.isIstRaum()){
			...
			System.out.println("anfangsRaum  : " +anfangsRaum.getName());
			
			spielerAnfangsPositionKachel = raumBeweger.positionInRaumMitPlayer(player, anfangsRaum);
			spielerAnfangsPositionLabel = boardView.getLabelHier(spielerAnfangsPositionKachel);
		
		}
		else {
			spielerAnfangsPositionKachel = spielerPositionKachel;
			spielerAnfangsPositionLabel = boardView.getLabelHier(spielerPositionKachel);
		}
		
		System.out.println("spielerAnfangsPositionKachel : y : " +spielerAnfangsPositionKachel.getPosition().getY() + "  ||  x : " +spielerAnfangsPositionKachel.getPosition().getX());
		
		System.out.println("zielRaum : " +zielRaum.getName());
		
		Kachel spielerPositionInDerTuerKachel = raumBeweger.positonInDerTuer(zielRaum);
		
		player.setNewPosition(spielerPositionInDerTuerKachel.getPosition());
		
		System.out.println("spielerPositionImRaumKachel y : " +spielerPositionInDerTuerKachel.getPosition().getY() + "  ||  x : " +spielerPositionInDerTuerKachel.getPosition().getX());
		
		Kachel spielerZielPositionImRaumKachel = raumBeweger.positionInRaumMitPlayer(player, zielRaum);
	...
	public void getCarriedAlongWeapon(CluedoGame network,Rooms zielRaum, CluedoWeapon waffe){

		System.out.println("get carried??!?!?!?!??!?!? (weapon)");
		
		Kachel waffenAnfangsPositionKachel = kachelContainer.getKachelAt(waffe.getPosition().getY(), waffe.getPosition().getX());		
		Label waffenAnfangsPositionLabel = boardView.getLabelHier(waffenAnfangsPositionKachel);
		
		System.out.println("waffenAnfangsPositionKachel : y : " +waffenAnfangsPositionKachel.getPosition().getY() + "  ||  x : " +waffenAnfangsPositionKachel.getPosition().getX());
		
		System.out.println("zielRaum : " +zielRaum.getName());
		
		Kachel waffenZielPositionKachel = raumBeweger.positionImRaumWaffe(zielRaum);
		Label waffenZielPositionLabel = boardView.getLabelHier(waffenZielPositionKachel);

		waffe.setNewPosition(waffenZielPositionKachel.getPosition());
		
		System.out.println("waffenZielPositionKachel y : " +waffenZielPositionKachel.getPosition().getY() + "  ||  x : " +waffenZielPositionKachel.getPosition().getX());
		
		Path path = new Path();
		path.getElements().add(new MoveTo(waffenAnfangsPositionLabel.getLayoutX(), waffenAnfangsPositionLabel.getLayoutY()));
		path.getElements().add(new LineTo(waffenZielPositionLabel.getLayoutX(), waffenZielPositionLabel.getLayoutY()));
		PathTransition pathTransition = new PathTransition();
		pathTransition.setDuration(Duration.millis(2000));
		pathTransition.setNode(pcManager.getWeaponImageByWeapon(waffe));
		pathTransition.setPath(path);
		pathTransition.play();

	}

	public void waffenAnfangsPositionenSetzen(int iEingabe){
			zaehlerWaffen = iEingabe;
			int size = pcManager.weapons.size();
			if (zaehlerWaffen < size){
			
				auxx.logsevere("anfangspositionen");
				Kachel startKachel = kachelContainer.getKacheln()[0][11];
				Label startLabel = boardView.getLabelHier(startKachel);
				
				Kachel zielKachel = kachelContainer.getKacheln()[11][11];
				Label zielLabel = boardView.getLabelHier(zielKachel);
				
				Path path = new Path();
				path.getElements().add(
						new MoveTo(startLabel.getLayoutX(), startLabel.getLayoutY()));
				path.getElements().add(
						new LineTo(zielLabel.getLayoutX(), zielLabel.getLayoutY()));

				PathTransition pathTransition = new PathTransition();
				pathTransition.setDuration(Duration.millis(1000));
				pathTransition.setNode(pcManager.weapons.get(zaehlerWaffen));
				pathTransition.setPath(path);
				...
				pathTransition.play();
					...
					pathTransition.setOnFinished(new EventHandler <ActionEvent> (){
						...
						public void handle(ActionEvent event) {
							zaehlerWaffen++;
							waffenAnfangsPositionenSetzen(zaehlerWaffen);
						}
			...
			pathTransition.setOnFinished(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					if ((schritte == 0 && spielerPositionKachel.isIstTuer()) || (schritte != 0 && nullSchritte != 0 && spielerPositionKachel.isIstTuer())) {
						...
						Label anfangsRaumLabel = boardView.getLabelHier(anfangsRaumKachel);
						Rooms room = raumBeweger.checkRaum(anfangsRaumKachel);
						zugFenster.setZimmer(room.getName().toUpperCase());
						Kachel raumZielKachel = raumBeweger.positionInRaum(room);
	...
	public void geheimgangBewegerEingang(Rooms anfangsRaum) {
		...
		Kachel gangKachel1_1 = raumBeweger.positionInRaum(anfangsRaum);
		...
		pathTransition.setOnFinished(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				Kachel gangKachel3 = raumBeweger.positionInRaum(room2_1);
				...
				zugFenster.setZimmer(room2.getName().toUpperCase());
//				stack.getChildren()
//				.add(zug);
//				zug.toFront();
			}
			...
			pathTransition.play();
				...
				pathTransition.setOnFinished(new EventHandler <ActionEvent> (){
					...
					public void handle(ActionEvent event) {
						...
						if (zaehler == size-1){
							waffenAnfangsPositionenSetzen(0);
						}						 
					}
