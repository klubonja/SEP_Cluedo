...
//		Stop transparent = new Stop(1, Color.TRANSPARENT);
        ...
        audio1 = new AudioClip(getClass().getResource("clientAudio1.mp3").toExternalForm());
        ...
        volumeBox.setPadding(new Insets(0, 20 , 0 , 0));
		 audio1.volumeProperty().bind(volume.valueProperty());
...
//	    grid.add(messagesIn,			1, 3, 	2, 		1);
	...
	public void setMenuBar(){
		...
		ddos = new MenuItem("start ddos");

		testServer = new MenuItem("Test LMU Server");
		exitMenu = new MenuItem("Quit");
		file.getItems().addAll(createGameMenu, createServerMenu, testServer,ddos, exitMenu);
//		file.getItems().add(options);
		...
		help.getItems().addAll(gameRules, faq);
...
//	}
	...
	public void setFaq(MenuItem faq) {
		this.faq = faq;
	}
